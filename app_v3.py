...
# =================== 页面标题 ====================
st.title("SCOPE: Student COgnitive Perception and Extrapolation")
    ...
    if not video_list:
        ...
        if selected_video:
            ...
            with tabs[0]:
                ...
                st.video(f"videos/{video_id}_AU_annotated_final.mp4")
                st.markdown(""" The facial action analysis shows:
- **AU3 (Brow Lowerer)** - 72.13%, associated with concentration, negative affect, or emotional tension.  
- **AU5 (Upper Lid Raiser / Cheek Raiser)** - 3.28%, occasionally linked to surprise, alertness, or emotional arousal.  
- **AU22 (Lips Part)** - 1.64%, rare lip parting indicating limited speech or passive engagement.
""")

            with tabs[1]:
                st.markdown("####  Valence-Arousal (VA) Analysis")
                st.image(f"videos/{video_id}_VA_plot.png", caption="Valence-Arousal Over Time")
                st.markdown("""
...
""")
            ...
            with tabs[2]:
                ...
                st.video(f"videos/eyeblink_{video_id}.mp4")
                st.markdown("""
A total of **3 eye blinks** were detected during the session:  
- **Blink 1**: Detected between **1.57s – 1.77s**, duration **0.2 seconds**  
- **Blink 2**: Detected between **4.00s – 4.20s**, duration **0.2 seconds**  
- **Blink 3**: A brief and subtle blink, considered negligible for behavioral interpretation.  

Overall, low frequency and fast blink suggests focused attention and cognitive engagement, as learners often suppress blinking to maintain visual continuity and mental concentration.
""")

            with tabs[3]:
                st.markdown("####  Gaze Tracking")
                st.video(f"videos/gaze_{video_id}.mp4")
